...
router.get('/', auth, async (req, res) => {
    try {
        ...
        const [logs] = await pool.query('SELECT * FROM habit_logs WHERE user_id = ?', [req.user.id]);
...
router.post('/', auth, async (req, res) => {
    try {
        ...
        await pool.query(
            ...
            [id, req.user.id, name, goals, description, category, icon_html, evaluation, unit]
        );
...
router.put('/:id', auth, async (req, res) => {
    try {
        ...
        await pool.query(
            ...
            [name, goals, description, category, icon_html, evaluation, unit, req.params.id, req.user.id]
        );
    ...
    try {
        ...
        await pool.query(
            `INSERT INTO habit_logs (user_id, habit_id, log_date, completed, numeric_value, timer_elapsed) 
             ...
             updated_at = NOW()`,
            [req.user.id, habit_id, log_date, completed, numeric_value, timer_elapsed]
        );
